...
# Getting postion and otehr parameters with get attributes
...
SP = nx.multi_source_dijkstra_path(network_graph, sources={'HB_Pirchanger', 'HB_Schmadl', 'HB_Pertrach', 'HB_Kraken'}, weight='Wei')
L = dict()
...
# Iteration after removing an edge; in C_max is edge 20, 10 missing -> pump -> no diameter -> no C_max  
        ...
        SP_abnormal = nx.multi_source_dijkstra_path(network_graph_2, sources={'HB_Pirchanger', 'HB_Schmadl', 'HB_Pertrach', 'HB_Kraken'}, weight='Wei')
...
Most_critical_pipes = sorted(M_failure_names, key=M_failure_names.get, reverse = False)[:5]
...
demand_impacted_output.to_csv("demand_impacted_graph.csv", sep=';')
